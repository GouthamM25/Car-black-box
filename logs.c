 ...
 * Author: GOUTHAM M
 *
 * Created on 9 January, 2025, 2:29 PM
 */
...
void view()//view log
{
    ...
    {
        CLEAR_DISP_SCREEN;
        clcd_print("NO Logs",LINE1(0));//no logs found
    ...
    if(flag)
    {
      ...
      for(char j=0;j<14;j++)
      {
        ...
        {
            data[j] = read_ext_eeprom((k*10)+ind++);
        }
      }
      data[14]='\0';
    }
...
void download_log()//download log
{
    char k=0;
    char data[15];
...
void get_time(void)//get time function
{
...
int min, hour, sec;
void set_time(void) //function to set time
{
